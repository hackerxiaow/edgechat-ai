...
import { getUserByUsername, getUserByIdentifier, getUserByEmail, listActiveUsers } from './data/users.ts';
import { createOpenUser, createPasswordReset, getPasswordReset, clearPasswordReset, resetUserPassword, generateVerificationCode, verifyVerificationCode, sendEmail } from './data/auth.ts';
...
app.post('/api/auth/send-code', async (c) => {
  ...
  if (!email) return errorResponse('请输入邮箱');
  if (purpose !== 'register' && purpose !== 'login') return errorResponse('无效的请求');

  const settings = await getRuntimeSettings(c.env.DB);
  if (!settings.smtpRelayUrl) return errorResponse('系统未配置发件服务，无法发送验证码');
  if (purpose === 'register' && !settings.allowOpenRegistration) return errorResponse('未开启开放注册');

  const user = await getUserByEmail(c.env.DB, email);
  if (purpose === 'register' && user) return errorResponse('该邮箱已被注册');
  if (purpose === 'login' && !user) return errorResponse('该邮箱未注册');

  const code = await generateVerificationCode(c.env.DB, email, purpose);
  const sent = await sendEmail(
    settings.smtpRelayUrl, 
    settings.smtpApiKey, 
    email, 
    `[${settings.siteName}] ${purpose === 'login' ? '登录' : '注册'}验证码`, 
    `你的验证码是：${code}\n\n该验证码在10分钟内有效。如果你没有请求此验证码，请忽略这封邮件。`
  );
  
  if (!sent) return errorResponse('邮件发送失败，请联系管理员核对发件配置');
  return c.json({ ok: true });
});
...
app.post('/api/auth/register-open', async (c) => {
  ...
  if (!username || !password || !email || !code) return errorResponse('用户名和密码不能为空');
  
  const isValidCode = await verifyVerificationCode(c.env.DB, email, code, 'register');
  if (!isValidCode) return errorResponse('验证码无效或已过期');
  const hashed = await hashPassword(password);
...
app.post('/api/auth/login', async (c) => {
  const payload = await parseJsonRequest(c.req.raw);
  const loginMethod = String(payload.method || 'password');
  ...
  if (loginMethod === 'code') {
    if (!email || !code) return errorResponse('请输入邮箱和验证码');
    const isValid = await verifyVerificationCode(c.env.DB, email, code, 'login');
    if (!isValid) return errorResponse('验证码无效或已过期');
    user = await getUserByEmail(c.env.DB, email);
  } else {
    if (!account || !password) return errorResponse('请输入账号和密码');
    user = await getUserByIdentifier(c.env.DB, account);
  }
  ...
  if (!user) {
    return errorResponse(loginMethod === 'code' ? '账号不存在' : '账号或密码错误', 401);
  }
  ...
  if (loginMethod !== 'code') {
    const isMatch = await verifyPassword(password, user.password_hash, user.password_salt);
    if (!isMatch) {

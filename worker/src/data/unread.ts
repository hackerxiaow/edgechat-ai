...
async function resolveReadableMessageId(
	db: D1Database,
	channelId: number | string,
	messageId: number | string | null = null,
): Promise<number> {
	const filters = ["channel_id = ?", "deleted_at IS NULL"];
	const binds: number[] = [Number(channelId)];
	...
	const { results } = await db
		.prepare(
			`SELECT COALESCE(MAX(id), 0) AS message_id FROM messages WHERE ${filters.join(" AND ")}`,
		)
		.bind(...binds)
...
export async function markRoomRead(
	db: D1Database,
	{ channelId, userId, messageId = null }: RoomReadInput,
): Promise<number> {
...
export async function countUnreadMessages(
	db: D1Database,
	{ channelId, userId }: { channelId: number | string; userId: number | string },
): Promise<number> {
	const { results } = await db
		.prepare(
		...
		.all<{ unread_count: number }>();
	return Number(results[0]?.unread_count || 0);
}

export async function countUnreadAttention(
	db: D1Database,
	{ channelId, userId }: { channelId: number | string; userId: number | string },
): Promise<number> {
	const { results } = await db
...
export async function listRoomMemberIds(
	db: D1Database,
	channelId: number | string,
): Promise<number[]> {
	const { results } = await db

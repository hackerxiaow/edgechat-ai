import { tokenizeMentionText, type MentionTextToken } from "./mentions.ts";
...
export function tokenizeWebLinks(text: string): MessageTextToken[] {
	const tokens: MessageTextToken[] = [];
	let cursor = 0;

	for (const match of text.matchAll(WEB_LINK_PATTERN)) {
		const offset = match.index;
		if (offset === undefined || !isLinkBoundary(text[offset - 1])) continue;
...
export function tokenizeMessageText(
	content: string,
	mentions: Parameters<typeof tokenizeMentionText>[1],
): MessageTextToken[] {

const fixedErrorTranslations = new Map([
  ...
  ['请输入 Bot Token', 'Please enter a Bot token'],
	  ...
	  ['API 接口地址不能为空', 'API endpoint URL cannot be empty'],
	  ['模型名称不能为空', 'Model name cannot be empty'],
	  ['外部图床地址必须是 https 链接', 'The external upload URL must be an https link'],
	  ['存储服务不可用，无法上传附件', 'Storage service is unavailable, so attachments cannot be uploaded'],
	  ['头像文件不存在或正在清理，请重新上传', 'The avatar file does not exist or is being cleaned up. Please upload it again'],
	  ['站点图标文件不存在或正在清理，请重新上传', 'The site icon file does not exist or is being cleaned up. Please upload it again'],
	  ['会话参数无效', 'Invalid conversation parameters'],
	  ['无效的请求', 'Invalid request'],
	  ['未开启开放注册', 'Open registration is disabled'],
	  ['注册失败，用户名或邮箱可能已存在', 'Registration failed. The username or email may already exist'],
	  ['刷新凭据无效', 'Invalid refresh credential'],
	  ['刷新凭据已失效，请重新登录', 'The refresh credential has expired. Please log in again'],
	  ['刷新凭据已被使用，请重新登录', 'The refresh credential has already been used. Please log in again'],
	  ['设备信息无效', 'Invalid device information'],
	  ['同步游标已过期，请重新加载会话', 'The sync cursor has expired. Please reload the conversation'],
	  ['当前会话不支持实时票据', 'This conversation does not support realtime tickets'],
	  ['当前部署不支持 WebSocket，请改用轮询同步', 'This deployment does not support WebSocket. Use polling sync instead'],
	  ['发言权限取值无效', 'Invalid posting permission value'],
	  ['历史消息可见性取值无效', 'Invalid history visibility value'],
	  ['慢速模式间隔取值无效', 'Invalid slow mode interval value'],
	  ['你不是该群组成员', 'You are not a member of this group'],
	  ['该用户不是群组成员', 'That user is not a member of this group'],
	  ['群主不能退出群组，请先转让群主或删除群组', 'The group owner cannot leave. Transfer ownership or delete the group first'],
	  ['只有群主可以转让群组', 'Only the group owner can transfer the group'],
	  ['不能转让给自己', 'You cannot transfer the group to yourself'],
	  ['请选择要转让的成员', 'Please select the member to transfer ownership to'],
	  ['general 系统群组不能退出', 'You cannot leave the general system group'],
	  ['general 系统群组不能转让', 'The general system group cannot be transferred']
]);
